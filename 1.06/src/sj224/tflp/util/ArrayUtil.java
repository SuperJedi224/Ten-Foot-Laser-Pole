...
public class ArrayUtil {
	private ArrayUtil() {}
	/**
	 * Filters an array according to a predicate. This will remove any elements which do not satisfy the predicate.
	 * @param array The array to filter
	 * @return A copy of the array filtered according to the predicate.
	 */
	public static <T> T[]filter(T[]array,Predicate<T>pred){
		...
		return ar.toArray(Arrays.copyOf(array, 0));
	}
	/**
	...
	/**
	 * Deep compares two arbitrary arrays. If either object is not an array, compares them according to the conventions of Object.equals().
	...
	public static boolean arrayEquals(Object array1,Object array2){
		int i=0;
		try{i=Array.getLength(array1);
		if(Array.getLength(array2)!=i)return false;}catch(IllegalArgumentException e){
			return Objects.equals(array1, array2);
		}
		for(int j=0;j<i;i++){
			if(!arrayEquals(Array.get(array1, j),Array.get(array2, j)))return false;
		}
		return true;
	}
	/**
	...
	/**
	 * Constructs a deep copy of an arbitrary array.
	...
	public static <T> T deepCopy(Object array){
		int i=Array.getLength(array);
		T o=(T)Array.newInstance(array.getClass().getComponentType(),i);
		for(int j=0;j<i;j++){
			try{
				Array.set(o,j,deepCopy(Array.get(array,j)));
			}catch(IllegalArgumentException e){
				Array.set(o,j,Array.get(array,j));
			}
		}
		return o;
	}
